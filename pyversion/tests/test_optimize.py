...
def test_fidelity_grad():
    ...
    # print(grad["local_bias"])
    assert grad['kernel'][0, 0] == pytest.approx((
        1.5j) - ((0.5+2j)*np.exp((0.5-2j)-(-0.5-1j)) + (-0.5+1j)*np.exp((1.5+1j)-(2.0+3j))) / term2_down / 2)


def test_sr_H_update():
    N = 8
    hi = nk.hilbert.Qubit(N)
    # define an arbitrary model, just for define the mcmc model
    model = rbm.RBM_flexable(N, N, rngs=jax.random.PRNGKey(15))
    ...
    phi_psi = np.array([[0.5-2j], [1.5+1j]])  # 2 samples
    psi_psi = np.array([[-0.5-1j], [2.0+3j]])  # 2 samples
    psi_phi = np.array([[0.3-1j], [0.7+2j]])  # 2 samples
    phi_phi = np.array([[0.1-2j], [0.9+3j]])  # 2 samples

    F, delta_params = opmodel.sr_H_update(
        psi_phi, phi_phi, psi_psi, phi_psi, O, 1e-3)
    print(F)
    print(delta_params['kernel'])
    print(delta_params['bias'])
    print(delta_params['local_bias'])
...
def test_hadamard_gate():
    ...
    model = rbm.RBM_flexable(N, N, rngs=jax.random.PRNGKey(15))
    np.random.seed(51)
    bias = 0.01*(np.random.rand(N) + 1j * np.random.rand(N))
    local_bias = 0.01*(np.random.rand(N) + 1j * np.random.rand(N))
    ...
    print("Stochastic reconfiguration time: ", end-start)
...
def test_mcmc_fidelity():
    ...
    vstate_psi = nk.vqs.MCState(sampler_psi, model, n_samples=2**13*4)
    ...
    print(samples_phi.shape)
    print(samples_psi.shape)
