...
def basis_convert(state: RBM_flexable, compairing_exact_state=None):
    ...
    # if mpi.rank == 0:
    #     init_state = np.random.choice([0, 1], comb_state.in_features)
    # else:
    #     init_state = None
    # init_state = MPI.COMM_WORLD.bcast(init_state, root=0)
    # comb_state.expand_dims(comb_state.out_features*2)  # expand until alpha=2
    op_model = mcmc_optimize(comb_state, nk.hilbert.Qubit(2*N), 8, 2*N, 2**13)
    overlap_history = []
    for i in range(N, 2*N):
        if mpi.rank == 0:
            print("Step One, Qubit: ", i)
        op_model.stochastic_reconfiguration_H(
            i, resample_phi=2, outprintconfig=5, lr_tau=0.5, lr_nstep=50, lr_min=0.005)
        comm.Barrier()
        if compairing_exact_state is not None:
    ...
    for i in range(2*N):
        if mpi.rank == 0:
            print("Step Three, Qubit: ", i)
        op_model.stochastic_reconfiguration_H(
            i, resample_phi=5, outprintconfig=5, lr_tau=0.5, lr_nstep=50, lr_min=0.005)
        comm.Barrier()
        if compairing_exact_state is not None:
            compairing_exact_state = hadamard(compairing_exact_state, i)
            approx_amplitude = np.exp(op_model.model(all_state))
...
def bell_magic(state: RBM_flexable, n_chains: int, n_sweeps=None, n_samples=2**13, rndkey=1, compairing_exact_state=None):
    ...
    sampler_nv_state = sampler_nv.init_state(nv, 1, rndkey)
